...
# 遍历以 job_dir_prefix 开头的结构目录
for id_dir in os.listdir(r_dir):
    if not id_dir.startswith(job_dir_prefix):
        continue

    if not os.path.isdir(os.path.join(r_dir, id_dir)):
        continue
    ...
    # 判断哪些轴有数据
    axes = []
    if "result-energy-vs-strain_0_DFT.txt" in files and "result-energy-vs-strain_0_NEP.txt" in files:
        axes.append('0')
    if "result-energy-vs-strain_1_DFT.txt" in files and "result-energy-vs-strain_1_NEP.txt" in files:
        axes.append('1')
    if "result-energy-vs-strain_2_DFT.txt" in files and "result-energy-vs-strain_2_NEP.txt" in files:
        axes.append('2')
    ...
    # 设置不同的 marker 对应不同的轴
    marker_DFT = {'0': 'o', '1': 's', '2': 'D'}  # DFT 轴的 marker：圆形，方形，菱形
    marker_NEP = {'0': '+', '1': 'x', '2': '*'}  # NEP 轴的 marker：加号，叉号，星号

    # 设置不同数字轴对应的实际晶轴标签
    axis_label = {'0': 'a', '1': 'b', '2': 'c'}
    ...
    for axis in axes:
        ...
        if os.path.exists(r_f_DFT) and os.path.exists(r_f_NEP):
            ...
            # 根据轴确定线型和颜色
            label_DFT = f"DFT-{axis_label[axis]}"
            label_NEP = f"NEP-{axis_label[axis]}"
    ...
    # 设置标题和坐标轴标签
    plt.title(f'{id_dir.removeprefix(job_dir_prefix)}', fontsize=fs)
