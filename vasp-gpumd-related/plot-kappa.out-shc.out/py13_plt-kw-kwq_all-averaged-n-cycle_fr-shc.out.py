...
# PS: This script groups all jobs with the same x and plots averaged kw only.
...
def calc_quantum_correction(nu, T):
    ...
    f[mask] = x[mask]**2 * np.exp(x[mask]) / (np.exp(x[mask]) - 1.0)**2

    return f


def calc_quantum_corrected_kappa(nu, kw, T):
    f = calc_quantum_correction(nu, T)
    kw_q = kw * f
    kappa_q_integrated = trapezoid(kw_q, nu)

    return kw_q, kappa_q_integrated
...
def plot_grouped_kw(x_val, nu_ref, kw_all, kw_q_all, kappa_all, kappa_q_all):
    kw_mean = np.mean(kw_all, axis=0)
    kw_q_mean = np.mean(kw_q_all, axis=0)

    kappa_mean = np.mean(kappa_all)
    kappa_q_mean = np.mean(kappa_q_all)

    if len(kappa_all) > 1:
        kappa_stderr = np.std(kappa_all, ddof=1) / np.sqrt(len(kappa_all))
        kappa_q_stderr = np.std(kappa_q_all, ddof=1) / np.sqrt(len(kappa_q_all))
    else:
        kappa_stderr = 0.0
        kappa_q_stderr = 0.0
    ...
    for kw_q in kw_q_all:
        plt.plot(nu_ref, kw_q, '-', color='red', alpha=0.2, linewidth=lw-1)
    ...
    plt.plot(
        nu_ref, kw_q_mean, 'r--', linewidth=lw,
        label=rf'$\kappa^q = {kappa_q_mean:.2f} \pm {kappa_q_stderr:.2f}$'
    )
    ...
    fname = os.path.join(
        w_dir,
        f'fig_kw-kwq_collected-average_{make_file_tag_x(x_val)}.png'
    )

    plt.savefig(fname, dpi=200)
    plt.close()

    return fname, kappa_mean, kappa_stderr, kappa_q_mean, kappa_q_stderr
...
def main():
    ...
    for x_val, job_list in sorted(groups.items()):
        ...
        for job, info in sorted(job_list):
            ...
            kw_q, kappa_q_integrated = calc_quantum_corrected_kappa(nu, kw, T)
            ...
            kw_all.append(kw)
            kw_q_all.append(kw_q)
            kappa_all.append(kappa_integrated)
            kappa_q_all.append(kappa_q_integrated)

            print(f"    kappa   = {kappa_integrated:.4f} W/m/K")
            print(f"    kappa_q = {kappa_q_integrated:.4f} W/m/K")
        ...
        kw_q_all = np.array(kw_q_all)
        kappa_all = np.array(kappa_all)
        kappa_q_all = np.array(kappa_q_all)

        fname, k_mean, k_se, kq_mean, kq_se = plot_grouped_kw(
            x_val, nu_ref, kw_all, kw_q_all, kappa_all, kappa_q_all
        )

        print(f"  averaged kappa   = {k_mean:.4f} +/- {k_se:.4f} W/m/K")
        print(f"  averaged kappa_q = {kq_mean:.4f} +/- {kq_se:.4f} W/m/K")
        print(f"Saved: {fname}")
